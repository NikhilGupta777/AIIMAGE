...
router.post("/chat/message", async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const msg of history) {
      ...
      const msgParts: Part[] = [{ text: msg.content }];
    ...
    res.status(500).json({ error: "Failed to get response from Gemini", details: errMsg });
  }
});
...
router.post("/chat/generate-image", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const errMsg = err instanceof Error ? err.message : "Unknown error";
    res.status(500).json({ error: "Failed to generate image", details: errMsg });
  }
});
